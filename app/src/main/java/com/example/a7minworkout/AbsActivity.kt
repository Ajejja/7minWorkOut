...
import com.example.a7minworkout.databinding.ActivityAbsBinding
import com.example.a7minworkout.databinding.DialogCostumBackBinding
import java.lang.Exception
import java.util.Locale

class AbsActivity : AppCompatActivity(),TextToSpeech.OnInitListener  {

    private var currentState: Bundle? = null // new variable to store current state
    private var isPaused = false
    private lateinit var ivPlay: ImageView
    private lateinit var ivPause: ImageView
    private var binding: ActivityAbsBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setSupportActionBar(binding?.abstoolbarExercise)
        ...
        exerciseList = AbsConstants.defaultExerciceList()
        tts = TextToSpeech(this, this)
    }

    

    private fun getCurrentTimeRemaining(): Long {
        return when {
            binding?.flRestView?.visibility == View.VISIBLE -> binding?.progressBar?.progress?.toLong() ?: 0
            binding?.flProgressBarEx?.visibility == View.VISIBLE -> binding?.progressBarEx?.progress?.toLong() ?: 0
            else -> 0
        }
    }
    ...
    private fun customDialogForBackButton() {
        ...
        private fun setupRestView() {
            ...
            exerciseList = exerciseList ?: AbsConstants.defaultExerciceList()
    ...
    private fun setProgressBar(timeRemaining: Long = 10000) {
        binding?.progressBar?.progress = restProgress
        restTimer = object : CountDownTimer(timeRemaining, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                if (restProgress == 9) {
                    ...
                    this@AbsActivity.timeRemaining = millisUntilFinished
                }
    ...
    private fun setExProgressBar(timeRemaining: Long = 30000) {
        binding?.progressBarEx?.progress = ExRestProgress
        ExRestTimer = object : CountDownTimer(timeRemaining, 1000) {
            override fun onTick(p0: Long) {
                ...
                if (ExRestProgress == 20) {
                    ...
                    this@AbsActivity.timeRemaining = p0
                }
            }
            ...
            override fun onFinish() {
                if (currentExercisePosition < exerciseList?.size!! - 1) {
                    ...
                    finish()
                    val intent = Intent(this@AbsActivity, FinalActivity::class.java)
